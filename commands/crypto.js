				...
				fetch(`http://api.shruc.ml/saladlog/price?coin=${args[0].toLowerCase()}`)
					...
					.then(json => {
						try {
							...
							if(!checkStatusVar == 1) newMessage.edit(`<:error:759701620777943062> Some funny stuff happened while attempting to display the data. Try again later. \`${error.message}\``);
							return;
						}
					});
			});
